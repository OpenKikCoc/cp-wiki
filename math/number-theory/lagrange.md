...
首先见群论部分的 [群论基础](math/group-theory.md) 有关群和元素的阶的定义，以及相关定理。
...
另外阅读后文 [原根](math/number-theory/primitive-root.md) 也可以知道，如果模 $m$ 的原根存在，那么自然也会满足上述性质“对于任意一个元素 $a$，至多有 $n$ 个不同的元素 $x$ 满足条件 $x^n\equiv a\pmod m$”。
