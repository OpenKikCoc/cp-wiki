...
2.  对于第 $i$ 个方程：
    1. 计算 $m_i=\frac{n}{n_i}$；
    2. 计算 $m_i$ 在模 $n_i$ 意义下的 [逆元](math/number-theory/inverse.md)  $m_i^{-1}$；
...
令 $r_{ij}$ 为 $p_i$ 在模 $p_j$ 意义下的 [逆](math/number-theory/inverse.md)：
...
否则，根据 [欧拉定理](math/number-theory/fermat.md)，可知所求为：
...
而计算一个组合数对较小的质数取模后的结果，可以利用 [卢卡斯定理](math/number-theory/lucas.md)。
