...
由于 $p$ 为奇素数，那么 $p-1$ 是一个偶数，根据 [费马小定理](math/number-theory/fermat.md)  $n^{p - 1} \equiv 1 \pmod{p}$，那么就有
...
$p$ 是一个奇素数，所以关于 $p$ 的 [原根](math/number-theory/primitive-root.md) 存在。

设 $a$ 是 $p$ 的一个 [原根](math/number-theory/primitive-root.md)，则存在 $1 \leqslant j \leqslant p-1$ 使得 $n=a^j$。于是就有

$$
a^{j\frac{p-1}{2}}\equiv1\pmod p
$$

$a$ 是 $p$ 的一个 [原根](math/number-theory/primitive-root.md)，因此 $a$ 模 $p$ 的指数是 $p-1$，于是 $p-1$ 整除 $\frac{j(p-1)}{2}$。这说明 $j$ 是一个偶数。
