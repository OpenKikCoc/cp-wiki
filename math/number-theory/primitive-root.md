...
前置：[费马小定理](math/number-theory/fermat.md)，[欧拉定理](math/number-theory/fermat.md)，[拉格朗日定理](math/number-theory/lagrange.md)。
...
由 [裴蜀定理](math/number-theory/bezouts.md) 得，一定存在一组 $k,x$ 满足 $kt=x\varphi(m)+\gcd(t,\varphi(m))$。

又由 [欧拉定理](math/number-theory/fermat.md) 得 $a^{\varphi(m)}\equiv 1\pmod{m}$，故有：
